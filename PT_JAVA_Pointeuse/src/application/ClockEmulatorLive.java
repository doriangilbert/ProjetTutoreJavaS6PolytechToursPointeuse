...
/**
 * Thread for the ClockEmulator class!
 * Every 10 seconds, calculates/refreshes the current time, date and the approximate time (to the nearest quarter hour).
 */
public class ClockEmulatorLive implements Runnable
{
	...
	public void run()
	{
		do
		{
			...
			Platform.runLater(() -> labelDate
					.setText(LocalDateTime.now().getDayOfWeek() + " "
	...
	/**
	 * If the number of minute is inferior to 10, add a 0 before the number to keep the same format:
	 * HH:MM instead of HH:M when minute < 10.
	 * 
